...
import pe.auna.siteds.beans.GetConsultaDatosAdiRequest;
...
public class ConsultaDatosAdiRequest extends AbstractTransformation {
    ...
    public void execute(InputStream in, OutputStream out) throws StreamTransformationException {
    	this.addInfoMessage("Iniciando Proceso de Mapeo: ConsultaDatosAdiRequest...");
    	try {
    		Document documentIn = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(in);
    		GetConsultaDatosAdiRequest getConsultaDatosAdiRequest = this.getConsultaDatosAdiRequest(documentIn);
    		Document documentOut = this.buildRequestMessage(getConsultaDatosAdiRequest);
        	TransformerFactory.newInstance().newTransformer().transform(new DOMSource(documentOut), new StreamResult(out));
        	this.addInfoMessage("Finalizando Proceso de Mapeo: ConsultaDatosAdiRequest...");
        } catch (Exception e) {
    ...
    public GetConsultaDatosAdiRequest getConsultaDatosAdiRequest(Document documentIn) {
    	String codigoExcepcion = this.getElementText(documentIn, "coExcepcion");
    	GetConsultaDatosAdiRequest getConsultaAsegNomRequest = new GetConsultaDatosAdiRequest();
    	getConsultaAsegNomRequest.setTxNombre(this.getElementText(documentIn, "txNombre"));
    	getConsultaAsegNomRequest.setCoIafa(this.getElementText(documentIn, "coIafa"));
    	getConsultaAsegNomRequest.setTxPeticion(codigoExcepcion.equals("0000") ? this.getElementText(documentIn, "txPeticion") : ""); 
    	getConsultaAsegNomRequest.setCoExcepcion(this.getElementText(documentIn, "coExcepcion"));
		return getConsultaAsegNomRequest;
    }
    
    private Document buildRequestMessage(GetConsultaDatosAdiRequest request) throws ParserConfigurationException {
        ...
        Element rootElement = documentOut.createElementNS(docNS, "ConsultaDatosAdiRequest");
