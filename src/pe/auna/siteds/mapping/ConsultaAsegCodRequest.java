...
import pe.auna.siteds.beans.GetConsultaAsegCodRequest;
...
public class ConsultaAsegCodRequest extends AbstractTransformation {
    ...
    public void execute(InputStream in, OutputStream out) throws StreamTransformationException {
    	this.addInfoMessage("Iniciando Proceso de Mapeo: ConsultaAsegCodRequest...");

    	try {
    		Document documentIn = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(in);
    		GetConsultaAsegCodRequest getConsultaAsegCodRequest = this.getConsultaAsegCodRequest(documentIn);
    		Document documentOut = this.buildRequestMessage(getConsultaAsegCodRequest);
        	TransformerFactory.newInstance().newTransformer().transform(new DOMSource(documentOut), new StreamResult(out));
        	this.addInfoMessage("Finalizando Proceso de Mapeo: ConsultaAsegCodRequest...");
        } catch (Exception e) {
    ...
    public GetConsultaAsegCodRequest getConsultaAsegCodRequest(Document documentIn) {
    	String codigoExcepcion = this.getElementText(documentIn, "coExcepcion");
    	GetConsultaAsegCodRequest getConsultaAsegCodRequest = new GetConsultaAsegCodRequest(); 		
        getConsultaAsegCodRequest.setTxPeticion(codigoExcepcion.equals("0000") ? this.getElementText(documentIn, "txPeticion") : "");       
    	getConsultaAsegCodRequest.setTxNombre(this.getElementText(documentIn, "txNombre"));
    	getConsultaAsegCodRequest.setCoIafa(this.getElementText(documentIn, "coIafa"));
    	getConsultaAsegCodRequest.setCoExcepcion(this.getElementText(documentIn, "coExcepcion"));
		return getConsultaAsegCodRequest;
    }
    
    private Document buildRequestMessage(GetConsultaAsegCodRequest request) throws ParserConfigurationException {
        ...
        Element rootElement = documentOut.createElementNS(docNS, "ConsultaAsegCodRequest");
