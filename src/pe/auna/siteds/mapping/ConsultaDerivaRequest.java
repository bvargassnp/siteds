...
import pe.auna.siteds.beans.GetConsultaDerivaRequest;
...
public class ConsultaDerivaRequest extends AbstractTransformation {
    ...
    public void execute(InputStream in, OutputStream out) throws StreamTransformationException {
    	this.addInfoMessage("Iniciando Proceso de Mapeo: ConsultaDerivaRequest...");

    	try {
    		Document documentIn = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(in);
    		GetConsultaDerivaRequest getConsultaDerivaRequest = this.getConsultaDerivaRequest(documentIn);
    		Document documentOut = this.buildRequestMessage(getConsultaDerivaRequest);
        	TransformerFactory.newInstance().newTransformer().transform(new DOMSource(documentOut), new StreamResult(out));
        	this.addInfoMessage("Finalizando Proceso de Mapeo: ConsultaDerivaRequest...");
        } catch (Exception e) {
    ...
    public GetConsultaDerivaRequest getConsultaDerivaRequest(Document documentIn) {
    	String codigoExcepcion = this.getElementText(documentIn, "coExcepcion");
    	GetConsultaDerivaRequest getConsultaDerivaRequest = new GetConsultaDerivaRequest();
    	getConsultaDerivaRequest.setTxNombre(this.getElementText(documentIn, "txNombre"));
    	getConsultaDerivaRequest.setCoIafa(this.getElementText(documentIn, "coIafa"));
    	getConsultaDerivaRequest.setTxPeticion(codigoExcepcion.equals("0000") ? this.getElementText(documentIn, "txPeticion") : "");
    	getConsultaDerivaRequest.setCoExcepcion(this.getElementText(documentIn, "coExcepcion"));
		return getConsultaDerivaRequest;
    }
    
    private Document buildRequestMessage(GetConsultaDerivaRequest request) throws ParserConfigurationException {
        ...
        Element rootElement = documentOut.createElementNS(docNS, "ConsultaDerivaRequest");
