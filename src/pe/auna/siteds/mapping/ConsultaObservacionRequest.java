...
import pe.auna.siteds.beans.GetConsultaObservacionRequest;
...
public class ConsultaObservacionRequest extends AbstractTransformation {
    ...
    public void execute(InputStream in, OutputStream out) throws StreamTransformationException {
    	this.addInfoMessage("Iniciando Proceso de Mapeo: ConsultaObservacionRequest...");

    	try {
    		Document documentIn = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(in);
    		GetConsultaObservacionRequest getConsultaObservacionRequest = this.getConsultaObservacionRequest(documentIn);
    		Document documentOut = this.buildRequestMessage(getConsultaObservacionRequest);
        	TransformerFactory.newInstance().newTransformer().transform(new DOMSource(documentOut), new StreamResult(out));
        	this.addInfoMessage("Finalizando Proceso de Mapeo: ConsultaObservacionRequest...");
        } catch (Exception e) {
    ...
    public GetConsultaObservacionRequest getConsultaObservacionRequest(Document documentIn) {
    	String codigoExcepcion = this.getElementText(documentIn, "coExcepcion");
    	GetConsultaObservacionRequest getConsultaObservacionRequest = new GetConsultaObservacionRequest();
    	getConsultaObservacionRequest.setTxNombre(this.getElementText(documentIn, "txNombre"));
    	getConsultaObservacionRequest.setCoIafa(this.getElementText(documentIn, "coIafa"));
    	getConsultaObservacionRequest.setTxPeticion(codigoExcepcion.equals("0000") ? this.getElementText(documentIn, "txPeticion") : ""); 
    	getConsultaObservacionRequest.setCoExcepcion(this.getElementText(documentIn, "coExcepcion"));
		return getConsultaObservacionRequest;
    }
    
    private Document buildRequestMessage(GetConsultaObservacionRequest request) throws ParserConfigurationException {
        ...
        Element rootElement = documentOut.createElementNS(docNS, "ConsultaObservacionRequest");
