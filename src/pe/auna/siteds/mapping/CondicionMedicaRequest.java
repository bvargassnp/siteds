...
import pe.auna.siteds.beans.GetCondicionMedicaRequest;
 ...
 *
 */
public class CondicionMedicaRequest extends AbstractTransformation {
    ...
    public void execute(InputStream in, OutputStream out) throws StreamTransformationException {
    	this.addInfoMessage("Iniciando Proceso de Mapeo: CondicionMedicaRequest...");
    	try {
    		Document documentIn = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(in);
    		GetCondicionMedicaRequest getCondicionMedicaRequest = this.getCondicionMedicaRequest(documentIn);
    		Document documentOut = this.buildRequestMessage(getCondicionMedicaRequest);
        	TransformerFactory.newInstance().newTransformer().transform(new DOMSource(documentOut), new StreamResult(out));
        	this.addInfoMessage("Finalizando Proceso de Mapeo: CondicionMedicaRequest...");
        } catch (Exception e) {
	...
	private GetCondicionMedicaRequest getCondicionMedicaRequest(Document documentIn) {
		String codigoExcepcion = this.getElementText(documentIn, "coExcepcion");
		GetCondicionMedicaRequest getCondicionMedicaRequest = new GetCondicionMedicaRequest();
		getCondicionMedicaRequest.setTxNombre(this.getElementText(documentIn, "txNombre")); 
		getCondicionMedicaRequest.setCoIafa(this.getElementText(documentIn, "coIafa"));
		getCondicionMedicaRequest.setTxPeticion(codigoExcepcion.equals("0000") ? this.getElementText(documentIn, "txPeticion") : ""); 
		getCondicionMedicaRequest.setCoExcepcion(this.getElementText(documentIn, "coExcepcion"));
		return getCondicionMedicaRequest;
	}
    
    private Document buildRequestMessage(GetCondicionMedicaRequest request) throws ParserConfigurationException {
        ...
        Element rootElement = documentOut.createElementNS(docNS, "CondicionMedicaRequest");
